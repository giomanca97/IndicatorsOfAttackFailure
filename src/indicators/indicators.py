...
def increasing_loss_indicator(atk_loss):
    ...
    for i in range(1, atk_loss.shape[0]):
        ...
        if v != 0:
            new_inc = [[(i - 1) / (atk_loss.shape[0]-1), i / (atk_loss.shape[0]-1)], [atk_loss[i - 1].item(), atk_loss[i].item()]]
            increasing += sklearn.metrics.auc(new_inc[0], new_inc[1])
    if increasing == 0:
        return 0, 0
    tot_auc_loss = sklearn.metrics.auc(np.linspace(0, 1, len(atk_loss)), atk_loss)
    auc_rel = increasing/tot_auc_loss
    st = None
    return auc_rel, st
